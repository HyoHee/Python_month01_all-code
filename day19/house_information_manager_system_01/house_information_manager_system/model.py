...
class HouseModel:
    ...
    def __str__(self):
        return (f"房屋编号是:{self.id},标题是:{self.title},所在小区是:{self.community},建造年份是:{self.years},房型是:{self.house_type},建筑面积是:{self.area},建筑底层是:{self.floor},描述信息是:{self.description},单价是:{self.unit_price},总价是:{self.total_price},带看记录是:{self.follow_info}")
